...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		...
		log("end is %p, nextfree is %p", end, nextfree);
	}
	...
	result = nextfree;
	if ((uintptr_t)nextfree + n > KERNBASE + PTSIZE) {
		panic("boot_alloc out of memory, n: %x\tnextfree: %x\n", n, nextfree);
	}
	nextfree += ROUNDUP(n, PGSIZE);
	log("alloc %d bytes, result is %p", n, result);
	return result;
}
...
mem_init(void)
{
	...
	n = ROUNDUP(envs_size, PGSIZE);
	assert(n < PTSIZE);
	log("UENVS: 0x%x, pages: 0x%x, size: %d", UENVS, PADDR(envs), n);
...
mem_init_mp(void)
{
	...
	uintptr_t vaddr = KSTACKTOP;
	for (int i = 0; i < NCPU; ++i) {
		vaddr -= KSTKSIZE;
		boot_map_region(kern_pgdir, vaddr, KSTKSIZE, PADDR(percpu_kstacks+i), PTE_W);
		vaddr -= KSTKGAP;
	}
}
...
page_free(struct PageInfo *pp)
{
	...
	// 	panic("page free parameter illegal!");
	// }
	assert(pp->pp_ref == 0);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (*pde_entry & PTE_P) {
		page_table = KADDR(PTE_ADDR(*pde_entry));
		return &page_table[PTX(va)];
	}
	if (!create) {
		log("pde entry doesn't exist, but not create. pdt: %p, va: %p", pgdir, va);
		return NULL;
	}
	page = page_alloc(ALLOC_ZERO);
	if (!page) {
		log("alloc page failed");
		return NULL;
	}
	page->pp_ref = 1;
	page->pp_link = NULL;
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	paddr = page2pa(pp);
	// prevent from freeing the same page, so add ref count before decrease it
	pp->pp_ref++;
	pp->pp_link = NULL;
	if (*pte & PTE_P) {
